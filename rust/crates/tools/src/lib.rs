...
fn preferred_tool_choice_for_text(
    ...
    let latest_user_text = latest_user_text.trim().to_ascii_lowercase();
    if latest_user_text.is_empty() {
        return None;
    }
    if is_freshness_query(&latest_user_text) {
...
fn is_freshness_query(latest_user_text: &str) -> bool {
    ...
    freshness_markers
        .iter()
        .any(|marker| latest_user_text.contains(marker))
}

pub fn adjusted_allowed_tools_for_conversation_messages(
    messages: &[ConversationMessage],
    available_tools: &BTreeSet<String>,
) -> BTreeSet<String> {
    let mut adjusted = available_tools.clone();
    ...
    let should_suspend_web_search = last_message.blocks.iter().any(|block| {
        matches!(
            block,
            ContentBlock::ToolResult { tool_name, .. } if tool_name == "WebSearch"
        )
    });
    if should_suspend_web_search {
        if latest_user_text
            .map(|text| is_freshness_query(&text.trim().to_ascii_lowercase()))
            .unwrap_or(false)
        {
            adjusted.clear();
        } else {
            adjusted.remove("WebSearch");
        }
    }
    adjusted
}

pub fn preferred_tool_choice_for_messages(
    ...
    preferred_tool_choice_for_text(latest_user_text, available_tools)
}

pub fn preferred_tool_choice_for_conversation_messages(
    messages: &[ConversationMessage],
    available_tools: &BTreeSet<String>,
) -> Option<ToolChoice> {
    let last_message = messages.last()?;
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "GitDiff",
            description: "Show git diff for the current workspace or staged changes.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "staged": { "type": "boolean" },
        ...
        ToolSpec {
            name: "GitPush",
            description: "Push the current branch to a git remote.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "remote": { "type": "string" },
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "GitCommit" => from_value::<GitCommitInput>(input).and_then(run_git_commit),
        "GitBranch" => from_value::<GitBranchInput>(input).and_then(run_git_branch),
        "GitPush" => from_value::<GitPushInput>(input).and_then(run_git_push),
        "GitPullRequest" => {
            from_value::<GitPullRequestInput>(input).and_then(run_git_pull_request)
        }
...
#[derive(Debug)]
struct SearchProviderRequest {
    label: &'static str,
    url: reqwest::Url,
    response_kind: SearchResponseKind,
}
...
fn execute_web_search(input: &WebSearchInput) -> Result<WebSearchOutput, String> {
    let started = Instant::now();
    let client = build_http_client()?;
    let search_requests = build_search_requests(input)?;
    ...
    for request in search_requests {
        let response = match client.get(request.url).send() {
            ...
            Err(error) => {
                errors.push(format!("{}: {error}", request.label));
                continue;
            }
        };
        ...
        let mut provider_hits = extract_search_hits_for_response(
        ...
        if provider_hits.is_empty() {
            errors.push(format!("{}: no parsable results", request.label));
            continue;
        }

        if let Some(allowed) = input.allowed_domains.as_ref() {
            provider_hits.retain(|hit| host_matches_list(&hit.url, allowed));
        }
        if let Some(blocked) = input.blocked_domains.as_ref() {
            provider_hits.retain(|hit| !host_matches_list(&hit.url, blocked));
        }

        dedupe_hits(&mut provider_hits);
        if !provider_hits.is_empty() {
            hits = provider_hits;
            break;
        }
...
fn build_search_requests(input: &WebSearchInput) -> Result<Vec<SearchProviderRequest>, String> {
    if let Ok(base) = std::env::var("CLAWD_WEB_SEARCH_BASE_URL") {
        let mut url = reqwest::Url::parse(&base).map_err(|error| error.to_string())?;
        url.query_pairs_mut().append_pair("q", &input.query);
        return Ok(vec![SearchProviderRequest {
            label: "custom-search-base",
            url,
            response_kind: SearchResponseKind::Auto,
        }]);
    }

    let mut requests = Vec::new();
    if is_news_query(&input.query) {
        if is_generic_japan_news_query(&input.query) {
            requests.push(SearchProviderRequest {
                label: "google-news-top-rss",
                url: build_google_news_top_rss_url()?,
                response_kind: SearchResponseKind::Rss,
            });
        } else if let Some(topic) = normalize_news_topic_query(&input.query) {
            requests.push(SearchProviderRequest {
                label: "google-news-search-rss",
                url: build_google_news_search_rss_url(&topic)?,
                response_kind: SearchResponseKind::Rss,
            });
            if topic == "日本" || topic.eq_ignore_ascii_case("japan") {
                requests.push(SearchProviderRequest {
                    label: "google-news-top-rss",
                    url: build_google_news_top_rss_url()?,
                    response_kind: SearchResponseKind::Rss,
    ...
    requests.push(SearchProviderRequest {
        label: "bing-search-rss",
        url: build_bing_search_rss_url(&input.query)?,
        response_kind: SearchResponseKind::Rss,
    });
    requests.push(SearchProviderRequest {
        label: "duckduckgo-html",
        url: build_duckduckgo_search_url(&input.query)?,
        response_kind: SearchResponseKind::Html,
    });
    Ok(requests)
}

fn build_bing_search_rss_url(query: &str) -> Result<reqwest::Url, String> {
    let mut url = reqwest::Url::parse("https://www.bing.com/search")
        .map_err(|error| error.to_string())?;
    let ascii_heavy = query.is_ascii();
    url.query_pairs_mut()
        .append_pair("format", "rss")
        .append_pair("q", query)
        .append_pair("cc", if ascii_heavy { "US" } else { "JP" })
        .append_pair("setlang", if ascii_heavy { "en-US" } else { "ja-JP" });
    Ok(url)
}

fn build_google_news_top_rss_url() -> Result<reqwest::Url, String> {
    reqwest::Url::parse("https://news.google.com/rss?hl=ja&gl=JP&ceid=JP:ja")
        .map_err(|error| error.to_string())
}

fn build_google_news_search_rss_url(query: &str) -> Result<reqwest::Url, String> {
...
fn build_duckduckgo_search_url(query: &str) -> Result<reqwest::Url, String> {
...
fn is_news_query(query: &str) -> bool {
    ...
    lowered.contains("news")
        ...
        || query.contains("ニュース")
        || query.contains("ヘッドライン")
        || query.contains("速報")
}

fn is_generic_japan_news_query(query: &str) -> bool {
    is_freshness_query(query)
        && is_news_query(query)
        && (query.contains("日本") || query.to_lowercase().contains("japan"))
}

fn normalize_news_topic_query(query: &str) -> Option<String> {
    ...
    let cleaned = normalized
        .chars()
        .map(|ch| {
            if ch.is_alphanumeric() || ch.is_whitespace() || ('\u{3040}'..='\u{30ff}').contains(&ch)
...
fn extract_search_hits_for_response(
    response_kind: SearchResponseKind,
    content_type: &str,
    body: &str,
    final_url: &reqwest::Url,
) -> Vec<SearchHit> {
    let looks_like_rss = content_type.contains("xml")
        || body.trim_start().starts_with("<?xml")
        || body.contains("<rss");
    if response_kind == SearchResponseKind::Rss
        || response_kind == SearchResponseKind::Auto && looks_like_rss
    {
        return extract_search_hits_from_rss(body);
    }

    let mut hits = extract_search_hits(body);
    if hits.is_empty() && final_url.host_str().is_some() {
        hits = extract_search_hits_from_generic_links(body);
    }
    hits
}

fn extract_search_hits_from_rss(xml: &str) -> Vec<SearchHit> {
    let mut hits = Vec::new();
    let mut remaining = xml;

    while let Some(item_start) = remaining.find("<item") {
        let after_item = &remaining[item_start..];
        let Some(item_open_end) = after_item.find('>') else {
            break;
        };
        let item_body = &after_item[item_open_end + 1..];
        let Some(item_end) = item_body.find("</item>") else {
            break;
        };
        let item = &item_body[..item_end];
        let title = extract_xml_tag_text(item, "title");
        let url = extract_xml_tag_text(item, "link");
        if let (Some(title), Some(url)) = (title, url) {
            let title = collapse_whitespace(&decode_html_entities(&title));
        ...
        remaining = &item_body[item_end + "</item>".len()..];
    }
...
fn extract_xml_tag_text(input: &str, tag: &str) -> Option<String> {
    ...
    let after = &input[start + open.len()..];
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    ...
    if let Ok(config_home) = std::env::var("SEBAS_CONFIG_HOME")
...
impl ApiClient for ProviderRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        let effective_allowed_tools =
            adjusted_allowed_tools_for_conversation_messages(&request.messages, &self.allowed_tools);
        let tools = tool_specs_for_allowed_tools(Some(&effective_allowed_tools))
            .into_iter()
            .map(|spec| ToolDefinition {
                name: spec.name.to_string(),
                description: Some(spec.description.to_string()),
            ...
            .collect::<Vec<_>>();
        let preferred_tool_choice = preferred_tool_choice_for_conversation_messages(
            &request.messages,
        ...
        let message_request = MessageRequest {
            ...
            tool_choice: preferred_tool_choice
                .or_else(|| (!effective_allowed_tools.is_empty()).then_some(ToolChoice::Auto)),
            stream: true,
        };
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".codex").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("SEBAS_CONFIG_HOME")
        .or_else(|_| std::env::var("CODEX_HOME"))
        .or_else(|_| std::env::var("CLAW_CONFIG_HOME"))
    {
...
mod tests {
    ...
    use super::{
        agent_permission_policy, allowed_tools_for_subagent, build_search_requests,
        execute_agent_with_spawn, execute_tool, extract_search_hits_from_rss,
        final_assistant_text, mvp_tool_specs, normalize_news_topic_query,
        permission_mode_from_plugin, persist_agent_terminal_state, push_output_block,
        AgentInput, AgentJob, SubagentToolExecutor, WebSearchInput,
    };
    ...
    #[test]
    fn rss_search_parser_extracts_titles_and_links() {
        let hits = extract_search_hits_from_rss(
            r#"<?xml version="1.0" encoding="utf-8"?>
            <rss version="2.0">
              <channel>
                <item>
                  <title><![CDATA[Reqwest docs]]></title>
                  <link>https://docs.rs/reqwest</link>
                </item>
                <item>
                  <title>Tokio guides</title>
        ...
        assert_eq!(hits[0].title, "Reqwest docs");
        assert_eq!(hits[0].url, "https://docs.rs/reqwest");
        assert_eq!(hits[1].title, "Tokio guides");
        assert_eq!(hits[1].url, "https://tokio.rs/tokio/tutorial");
    }

    #[test]
    fn generic_japan_news_queries_prefer_top_headlines_feed() {
        let requests = build_search_requests(&WebSearchInput {
            query: "今日の日本の最新ニュース10個".to_string(),
            allowed_domains: None,
            blocked_domains: None,
        })
        .expect("search requests should build");

        assert_eq!(requests[0].label, "google-news-top-rss");
        assert!(requests[0]
            .url
            .as_str()
            .starts_with("https://news.google.com/rss?"));
    }

    #[test]
    fn normalize_news_topic_query_strips_filler_words() {
        let normalized = normalize_news_topic_query("Latest Apple news headlines today")
