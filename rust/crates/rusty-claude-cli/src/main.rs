...
use serde_json::json;
use tools::{
    adjusted_allowed_tools_for_conversation_messages, execute_tool,
    preferred_tool_choice_for_conversation_messages, GlobalToolRegistry,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if try_handle_sebas_direct_commands(&raw_args)? {
        return Ok(());
    }
    let mut prepared = prepare_sebas_args(raw_args)?;
    let mut action = parse_args(&prepared.args)?;
    if prepared.engine_runtime.is_none()
        && !has_model_flag(&prepared.args)
        && matches!(action, CliAction::Prompt { .. } | CliAction::Repl { .. })
    {
        ...
        prepared.engine_runtime = Some(runtime);
    }
    if matches!(action, CliAction::Prompt { .. } | CliAction::Repl { .. }) {
        if let Some(runtime) = prepared.engine_runtime.as_ref() {
            ensure_engine_ready(runtime)?;
            runtime.export_env();
        }
    }
    match action {
...
struct PreparedCliArgs {
    args: Vec<String>,
    engine_runtime: Option<EngineRuntime>,
}
...
fn inject_model_flag(args: &mut Vec<String>, model: &str) {
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
            "--model" => {
...
fn extract_engine_from_model_flag(args: &[String]) -> Option<EngineKind> {
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
            "--model" => {
                let value = args.get(index + 1)?;
                return EngineKind::parse(value).ok();
            }
            flag if flag.starts_with("--model=") => {
                return EngineKind::parse(&flag["--model=".len()..]).ok();
            }
            _ => {
                index += 1;
            }
...
fn should_infer_default_engine(args: &[String]) -> bool {
    ...
    match first {
        "chat" | "prompt" => true,
        "help" | "version" | "engine" | "config" | "session" | "mcp" | "doctor" | "bench"
        | "model" | "search"
        | "status" | "sandbox" | "dump-manifests" | "bootstrap-plan" | "agents"
        | "skills" | "system-prompt" | "login" | "logout" | "init" => false,
        "--help" | "-h" | "--version" | "-V" | "--resume" => false,
        flag if flag.starts_with("--resume=") => false,
        flag if flag.starts_with('/') || flag.starts_with('-') => false,
        _ => true,
    }
}

fn prepare_sebas_args(
    raw_args: Vec<String>,
) -> Result<PreparedCliArgs, Box<dyn std::error::Error>> {
    let (explicit_engine, stripped) =
        extract_engine_arg(&raw_args).map_err(std::io::Error::other)?;
    let root_dir = workspace_root()?;

    let mut engine_runtime = explicit_engine
        .map(|engine| load_runtime(&root_dir, engine))
        .transpose()
        .map_err(std::io::Error::other)?;

    let mut args = stripped;
    if engine_runtime.is_none() {
        engine_runtime = extract_engine_from_model_flag(&args)
            .map(|engine| load_runtime(&root_dir, engine))
            .transpose()
            .map_err(std::io::Error::other)?;
    }
    if let Some(command) = args.first().map(String::as_str) {
        match command {
            ...
            "prompt" => {
                args = normalize_prompt_command_args(&args[1..])?;
            }
            "run" => {
                if let Some(profile) = args.get(1).map(String::as_str) {
                    if matches!(profile, "local-35b" | "local-122b") {
                        if engine_runtime.is_none() {
                            engine_runtime = Some(
                                load_runtime(
                                    &root_dir,
                                    EngineKind::parse(profile).map_err(std::io::Error::other)?,
                                )
                                .map_err(std::io::Error::other)?,
                            );
                        }
    ...
    if let Some(runtime) = engine_runtime.as_ref() {
        inject_model_flag(&mut args, &runtime.model_id());
    }

    Ok(PreparedCliArgs { args, engine_runtime })
}

fn normalize_prompt_command_args(
    args: &[String],
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            | "--model" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| std::io::Error::other(format!("missing value for {}", args[index])))?;
                options.push(args[index].clone());
                options.push(value.clone());
                index += 2;
            }
            "--dangerously-skip-permissions" | "--print" | "--help" | "-h" | "--version"
            | "-V" => {
                options.push(args[index].clone());
                index += 1;
            }
            flag
                if flag.starts_with("--output-format=")
    ...
    normalized.push(prompt_parts.join(" "));
    Ok(normalized)
}
...
fn resolve_engine_runtime(
    ...
    let engine =
        engine.ok_or_else(|| std::io::Error::other("missing engine. Use --engine qwen35b|qwen122b"))?;
...
fn try_handle_sebas_direct_commands(
    ...
    match command {
        ...
        "run" if args.get(1).map(String::as_str) == Some("engine-only") => {
            let (runtime, _) = resolve_engine_runtime(&args[2..], None)?;
        ...
        "engine" => {
            ...
            match action {
                ...
                "doctor" => {
                    print_engine_doctor(&runtime).map_err(std::io::Error::other)?;
                }
                "status" => {
                    print_engine_status(&runtime);
                }
                ...
                other => {
                    return Err(
                        std::io::Error::other(format!("unknown engine command: {other}")).into(),
                    );
                }
            }
            Ok(true)
        ...
        "doctor" => {
            let default_engine = args
                .get(1)
                .and_then(|value| EngineKind::parse(value).ok());
            let range = if default_engine.is_some() {
                &args[2..]
            } else {
                &args[1..]
            };
            let (runtime, _) = resolve_engine_runtime(range, default_engine)?;
            print_engine_doctor(&runtime).map_err(std::io::Error::other)?;
            Ok(true)
        }
        "bench" => {
        ...
        "config" => {
            match args.get(1).map(String::as_str) {
                Some("import-qwen") => {
                    println!("{}", import_qwen_settings_into_claw(&workspace_root()?)?);
                }
                section => println!("{}", render_config_report(section)?),
            }
            Ok(true)
        }
        "model" => {
            println!("{}", handle_model_command(&args[1..])?);
            Ok(true)
        }
        "search" => {
            println!("{}", handle_search_command(&args[1..])?);
            Ok(true)
        }
        "session" => {
            println!("{}", render_session_cli_report(args.get(1).map(String::as_str))?);
            Ok(true)
        }
        "mcp" => {
            println!("{}", render_mcp_report()?);
            Ok(true)
        }
        _ => Ok(false),
    }
...
fn resolve_model_alias(model: &str) -> &str {
    match model {
        ...
        "qwen35b" => "qwen3.5-35b-a3b-4bit",
        "qwen122b" => "qwen3.5-122b-4bit",
        _ => model,
    }
}
...
fn set_project_default_model(value: &str) -> Result<String, Box<dyn std::error::Error>> {
    let engine = EngineKind::parse(value).map_err(std::io::Error::other)?;
    let cwd = env::current_dir()?;
    let config_dir = cwd.join(".codex");
    ...
    root["model"] = json!(resolve_model_alias(engine.as_cli_label()));
    fs::write(&settings_path, serde_json::to_string_pretty(&root)?)?;
    Ok(format!(
        "Model updated\n  Active default    {}\n  Scope             project\n  Path              {}",
        resolve_model_alias(engine.as_cli_label()),
        settings_path.display()
    ))
}

fn preferred_tool_choice_for_request(
    request: &ApiRequest,
    ...
    let configured_tools = tool_registry
        .definitions(allowed_tools)
        .into_iter()
        .map(|tool| tool.name)
        .collect::<BTreeSet<_>>();
    let available_tools =
        adjusted_allowed_tools_for_conversation_messages(&request.messages, &configured_tools);
    preferred_tool_choice_for_conversation_messages(&request.messages, &available_tools)
        .or_else(|| (!available_tools.is_empty()).then_some(ToolChoice::Auto))
}
...
fn filter_tool_specs_for_request(
    ...
    let configured_names = configured_tools
        ...
        .collect::<BTreeSet<_>>();
    let available_tools =
        adjusted_allowed_tools_for_conversation_messages(&request.messages, &configured_names);
    configured_tools
        .into_iter()
        .filter(|tool| available_tools.contains(&tool.name))
        .collect()
}
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .codex/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
  ...
  Merged keys       {}",
    ...
    if let Some(section) = section {
        ...
        let value = match section {
            ...
            "mcp" => runtime_config.get("mcpServers"),
...
fn render_session_cli_report(action: Option<&str>) -> Result<String, Box<dyn std::error::Error>> {
    match action {
        None | Some("list") => render_session_list(""),
        Some("latest") => {
            let latest = latest_managed_session()?;
            Ok(format!(
                "Session
  ...
  Path             {}",
                ...
                format_session_modified_age(latest.modified_epoch_millis),
                latest.path.display(),
            ))
        }
...
fn render_mcp_report() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let loader = ConfigLoader::default_for(&cwd);
    let runtime_config = loader.load()?;
    ...
    for (name, server) in servers {
        ...
        lines.push(format!(
            "  {name:<18} transport={transport:?} scope={scope}",
...
fn import_qwen_settings_into_claw(
    root_dir: &Path,
) -> Result<String, Box<dyn std::error::Error>> {
    let source_path = root_dir.join(".qwen/settings.json");
    let source = fs::read_to_string(&source_path)?;
    let qwen: serde_json::Value = serde_json::from_str(&source)?;

    let codex_dir = root_dir.join(".codex");
    let codex_agents_dir = codex_dir.join("agents");
    fs::create_dir_all(&codex_agents_dir)?;

    let mut target = BTreeMap::new();
    if let Some(env) = qwen.get("env").and_then(serde_json::Value::as_object) {
        let env_map = env
            .iter()
            .filter_map(|(key, value)| {
                value
                    .as_str()
                    .map(|value| (key.clone(), RuntimeJsonValue::String(value.to_string())))
            })
            .collect::<BTreeMap<_, _>>();
        target.insert("env".to_string(), RuntimeJsonValue::Object(env_map));
    }
    if let Some(model_name) = qwen
        .get("model")
        ...
        .and_then(serde_json::Value::as_str)
    {
        target.insert(
            "model".to_string(),
            RuntimeJsonValue::String(model_name.to_string()),
        );
    }
    if let Some(approval_mode) = qwen
        .get("tools")
        .and_then(|value| value.get("approvalMode"))
        .and_then(serde_json::Value::as_str)
    {
        let permission_mode = match approval_mode {
            "auto-edit" | "accept-edits" | "acceptEdits" => "workspace-write",
            "read-only" => "read-only",
            _ => "danger-full-access",
        };
        target.insert(
            "permissionMode".to_string(),
            RuntimeJsonValue::String(permission_mode.to_string()),
        );
    }

    let mut sebas = BTreeMap::new();
    let mut engines = BTreeMap::new();
    for engine in [EngineKind::Qwen35b, EngineKind::Qwen122b] {
        if let Ok(runtime) = load_runtime(root_dir, engine) {
            let mut engine_config = BTreeMap::new();
            engine_config.insert(
                "modelDir".to_string(),
                RuntimeJsonValue::String(runtime.model_dir.display().to_string()),
            );
            engine_config.insert(
                "port".to_string(),
                RuntimeJsonValue::Number(i64::from(runtime.port)),
            );
            engine_config.insert(
                "baseUrl".to_string(),
                RuntimeJsonValue::String(runtime.openai_base_url()),
            );
            engine_config.insert(
                "modelId".to_string(),
                RuntimeJsonValue::String(runtime.model_id()),
            );
            engines.insert(
                runtime.engine.as_cli_label().to_string(),
                RuntimeJsonValue::Object(engine_config),
            );
        }
    }
    sebas.insert("engines".to_string(), RuntimeJsonValue::Object(engines));
    sebas.insert(
        "importedQwenJson".to_string(),
        RuntimeJsonValue::String(source.clone()),
    );
    target.insert("sebas".to_string(), RuntimeJsonValue::Object(sebas));

    let target_path = codex_dir.join("settings.json");
    fs::write(&target_path, RuntimeJsonValue::Object(target).render())?;

    let qwen_agents_dir = root_dir.join(".qwen/agents");
    let copied_agents = copy_markdown_tree(&qwen_agents_dir, &codex_agents_dir)?;

    Ok(format!(
        "Imported Qwen config
  Source           {}
  Target           {}
  Agents copied    {}",
        source_path.display(),
...
fn copy_markdown_tree(
    ...
    for entry in fs::read_dir(source_dir)? {
        ...
        if path.is_dir() {
            ...
            copied += copy_markdown_tree(&path, &destination)?;
            continue;
        }
...
fn describe_tool_progress(name: &str, input: &str) -> String {
    ...
    match name {
        ...
        "GitStatus" => "inspecting git status".to_string(),
        "GitDiff" => {
            if parsed
                .get("staged")
                ...
                .unwrap_or(false)
            {
                "inspecting staged git diff".to_string()
            } else {
                "inspecting git diff".to_string()
            }
        }
        "GitCommit" => parsed
            .get("message")
            .and_then(|value| value.as_str())
            .map_or_else(
                || "creating git commit".to_string(),
                |message| format!("commit {}", truncate_for_summary(message, 100)),
            ),
        "GitBranch" => parsed
            .get("name")
            .and_then(|value| value.as_str())
            .map_or_else(
                || "switching git branch".to_string(),
                |name| format!("branch {}", truncate_for_summary(name, 100)),
            ),
        "GitPush" => "pushing git branch".to_string(),
        "GitPullRequest" => "creating pull request".to_string(),
        _ => {
...
impl ApiClient for AnthropicRuntimeClient {
    ...
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        ...
        let message_request = MessageRequest {
            ...
            tool_choice: preferred_tool_choice_for_request(
                &request,
                &self.tool_registry,
                self.allowed_tools.as_ref(),
                self.enable_tools,
...
fn format_tool_call_start(name: &str, input: &str) -> String {
    ...
    let detail = match name {
        ...
        "web_search" | "WebSearch" => parsed
            ...
            .unwrap_or("?")
            .to_string(),
        "GitStatus" => "Inspecting git workspace status".to_string(),
        "GitDiff" => "Inspecting git diff".to_string(),
        "GitCommit" => parsed
            .get("message")
            .and_then(|value| value.as_str())
            .map_or_else(|| "Creating git commit".to_string(), |value| {
                format!("Creating git commit\n\x1b[2m{}\x1b[0m", truncate_for_summary(value, 120))
            }),
        "GitBranch" => parsed
            .get("name")
            .and_then(|value| value.as_str())
            .map_or_else(|| "Switching git branch".to_string(), |value| {
                format!("Switching git branch\n\x1b[2m{}\x1b[0m", truncate_for_summary(value, 120))
            }),
        "GitPush" => "Pushing git branch".to_string(),
        "GitPullRequest" => "Creating pull request".to_string(),
...
fn format_tool_result(name: &str, output: &str, is_error: bool) -> String {
    ...
    match name {
        ...
        "GitStatus" | "GitDiff" | "GitCommit" | "GitBranch" | "GitPush" | "GitPullRequest" => {
            format_generic_tool_result(icon, name, &parsed)
        }
...
fn format_web_search_result(icon: &str, parsed: &serde_json::Value) -> String {
    ...
    let mut lines = vec![format!(
        "{icon} \x1b[38;5;245mWebSearch\x1b[0m {} results",
        hits.len()
    )];
    if !query.is_empty() {
        lines.push(format!("\x1b[2mquery: {}\x1b[0m", truncate_for_summary(query, 120)));
    }
    ...
    for hit in hits.iter().take(5) {
        ...
        rendered_hits.push(format!("• {}", truncate_for_summary(title, 140)));
        if !url.is_empty() {
            rendered_hits.push(format!("  \x1b[2m{}\x1b[0m", truncate_for_summary(url, 160)));
        }
    }
    ...
    lines.push(rendered_hits.join("\n"));
    if hits.len() > 5 {
        lines.push("\x1b[2m… additional results preserved in session.\x1b[0m".to_string());
    }
    lines.join("\n")
}
...
UU conflicted.rs",
    ...
    #[test]
    fn tool_rendering_compacts_web_search_output() {
        let output = json!({
            ...
            "results": [
                "Search results",
                {
                    ...
                    "content": [
                        {
                            "title": "イラン攻撃激化をトランプ氏が予告 - 読売新聞オンライン",
                            "url": "https://news.google.com/rss/articles/one"
                        },
                        {
                            "title": "トランプ氏、司法長官を解任 - 時事ドットコム",
        ...
        let rendered = format_tool_result("WebSearch", &output, false);

        assert!(rendered.contains("WebSearch"));
        assert!(rendered.contains("2 results"));
        assert!(rendered.contains("日本の最新ニュース10個"));
        assert!(rendered.contains("イラン攻撃激化をトランプ氏が予告"));
        assert!(!rendered.contains("\"durationSeconds\""));
        assert!(!rendered.contains("\"tool_use_id\""));
    }
