...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            if let Some(content) = choice.delta.content.filter(|value| !value.is_empty()) {
                let visible = self.think_filter.feed(&content);
                if visible.is_empty() {
                    continue;
                }
                ...
                events.push(StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                    index: 0,
                    delta: ContentBlockDelta::TextDelta { text: visible },
                }));
            }
    ...
    fn finish(&mut self) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        let mut events = Vec::new();
        let trailing = self.think_filter.finish();
        if !trailing.is_empty() {
            ...
            events.push(StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                index: 0,
                delta: ContentBlockDelta::TextDelta { text: trailing },
            }));
        }
    ...
    fn feed(&mut self, chunk: &str) -> String {
        ...
        while !self.pending.is_empty() {
            if self.inside_think {
                if let Some(close_index) = self.pending.find(CLOSE_THINK) {
                    self.pending.drain(..close_index + CLOSE_THINK.len());
                    self.inside_think = false;
                    continue;
                }
                let keep = self.pending.chars().count().min(CLOSE_THINK.len().saturating_sub(1));
                let suffix = self
                    .pending
                    .chars()
                    .rev()
            ...
            if let Some(open_index) = self.pending.find(OPEN_THINK) {
                if open_index > 0 {
                    visible.push_str(&self.pending[..open_index]);
                }
                self.pending.drain(..open_index + OPEN_THINK.len());
                self.inside_think = true;
                continue;
            }

            let safe_len = self.pending.len().saturating_sub(OPEN_THINK.len().saturating_sub(1));
            if safe_len > 0 {
...
fn strip_think_tags(text: &str) -> String {
    ...
    let mut visible = filter.feed(text);
...
mod tests {
    ...
    #[test]
    fn strips_think_tags_from_non_streaming_text() {
        assert_eq!(
            strip_think_tags("<think>hidden</think>visible"),
            "visible"
        );
        assert_eq!(
            strip_think_tags("a<think>hidden</think>b"),
            "ab"
        );
    }

    #[test]
    fn think_filter_handles_split_tags_across_chunks() {
        let mut filter = ThinkTagFilter::default();
